...
class SpdController():
  ...
  def update_lead(self, CS,  dRel, yRel, vRel ):
    ...
    delta_ego = CS.clu_Vanz - lead_set_speed 
    if delta_ego > 5:
  ...
  def update(self, v_ego_kph, CS, sm, actuators, dRel, yRel, vRel, model_speed ):
    ...
    str3 = '도로곡률={:03.0f}  크루즈속도={:03.0f}  반응시간={:03.0f}  HEARTBEAT={:.0f} '.format( model_speed, target_set_speed, self.long_wait_timer,  tm_sample )
    trace1.printf2(  str3 )
